## Page Title
macOS and iOS Client Applications -- Open Source Attributions

## Summary
Third-party OSS license notices for the Twingate macOS and iOS clients. No technical implementation guidance -- legal/compliance reference listing component licenses.

## Key Information
- **Notable components and licenses**:
  - `libssl` (OpenSSL) -- Apache 2.0
  - `quickily` (QUIC implementation) -- MIT
  - `libevent`, `lwip` -- BSD 3-Clause
  - `siphash` -- CC0 1.0 Universal
  - `libjansson`, `jwt-cpp`, `args`, `pubnub`, `Sentry-cocoa` -- MIT
  - `nanopb`, `zlib` -- zlib License
  - `catch2` -- Boost Software License 1.0
  - `fmt` -- MIT-equivalent with optional exception

## Prerequisites
None.

## Step-by-Step
Not applicable.

## Configuration Values
None.

## Gotchas
None.

## Related Docs
- `/docs/open-source-attributions` -- index page
